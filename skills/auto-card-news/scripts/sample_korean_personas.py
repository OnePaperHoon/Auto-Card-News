#!/usr/bin/env python3
"""Sample real rows from NVIDIA Nemotron-Personas-Korea for copy QA."""
...
def as_markdown(rows: list[dict[str, Any]], topic: str | None) -> str:
    ...
    lines.extend(
        [
            "",
            "Use these real synthetic Korean personas as a reader panel. Do not quote private-looking details as if they are real people; use them to test whether the copy sounds natural, concrete, and worth stopping for.",
            "",
        ]
    )
    ...
    for index, row in enumerate(rows, start=1):
        lines.extend(
            [
                ...
                "",
                "Copy QA prompts:",
                "- Would this person understand the hook in 2 seconds?",
                "- Which word feels translated, corporate, or too technical?",
                "- What Korean phrase would make this feel closer to their life?",
                "- Would they save, share, comment, or just swipe away?",
                "",
            ]
        )
    return "\n".join(lines)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Sample real Nemotron-Personas-Korea rows for Korean copy QA.")
    ...
    parser.add_argument("--topic", default="", help="Optional content topic to include in the panel notes.")
